...
This folder contains all external dependencies that ChemaModsV2 requires, this README should explain some of the things you should know.
...
ChemaModsV2 contains many different dependencies and not all CMake scripts under this folder are used to fetch dependencies, some server a utilitarian function.

- `async-logger.cmake` : Library based on glog updated to support more modern C++ syntax
- `cpr.cmake` : Allows ChemaModsV2 to make connections to the internet and fetch data (mainly used for translations)
...
- `imgui.cmake` : The UI library used by ChemaModsV2
- `json.cmake` : Allows ChemaModsV2 to parse json files and load them in as native C++ objects
- `lua.cmake` : Self-explanatory, implements a Lua runtime and allows ChemaModsV2 to expose functions to Lua directly
