...
namespace big
{
	...
	std::string api_service::get_translation(std::string message, std::string target_language)
	{
		...
		else if (response.status_code == 0)
		{
			...
			LOG(WARNING) << "[Chat Translator]Unable to connect to LibreTranslate server. Follow the guide in ChemaModsV2 Wiki to setup LibreTranslate server on your computer.";
		}
		else
		{
