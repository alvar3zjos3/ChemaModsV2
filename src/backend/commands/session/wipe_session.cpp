...
namespace big
{
	class empty_session : command
	{
		...
		virtual void execute(const command_arguments&, const std::shared_ptr<command_context> ctx) override
		{
			...
			g_notification_service.push("VACIAR_SESION"_T.data(), "BACKEND_WIPE_SESSION_COMPLETE"_T.data());
		}
	};

	empty_session g_empty_session("emptysession", "VACIAR_SESION", "BACKEND_WIPE_SESSION_DESC", 0);
}
