...
namespace big
{
	...
	void looped::self_wanted()
	{
		...
		if (g_local_player && g_local_player->m_player_info)
		{
			...
			{
				// If the player hasn't updated the wanted level and we're not locking, then ChemaModsV2 should update its own slider value to reflect the actual in-game wanted level since it may have changed
