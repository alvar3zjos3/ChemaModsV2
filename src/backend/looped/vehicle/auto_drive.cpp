...
namespace big
{
	void looped::vehicle_auto_drive()
	{
		...
		if (!self::veh && current_destination != AutoDriveDestination::STOPPED)
		{
			...
			g_notification_service.push_warning("CONDUCCION_AUTOMATICA"_T.data(), "PLAYER_INFO_NO_VEHICLE"_T.data());
		}
		...
		if (current_destination != AutoDriveDestination::STOPPED)
		{
			...
			if (current_destination == AutoDriveDestination::EMERGENCY_STOP || (to_waypoint && !does_waypoint_exist) || interupted)
			{
				...
				{
					g_notification_service.push_warning("CONDUCCION_AUTOMATICA"_T.data(), "TELEPORT_NO_WAYPOINT_SET"_T.data());
				}
				else
				{
					g_notification_service.push_warning("CONDUCCION_AUTOMATICA"_T.data(), "BACKEND_LOOPED_VEHICLE_AUTO_DRIVE_STOPPED"_T.data());
				}
