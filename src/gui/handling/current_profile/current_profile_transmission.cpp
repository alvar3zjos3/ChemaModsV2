...
namespace big
{
	void tab_current_profile::tab_transmission()
	{
		if (ImGui::BeginTabItem("HANDLING_TAB_TRANSMISSION"_T.data()))
		{
			...
			if (ImGui::SliderFloat("##sesgo de tracción delantero", &fDriveBiasFront, 0.f, 1.0f))
			...
			if (ImGui::SliderFloat("##sesgo de tracción trasero", &fDriveBiasRear, 0.f, 1.0f))
			...
			ImGui::SliderFloat("##fuerza de conducción inicial", &g_local_player->m_vehicle->m_handling_data->m_initial_drive_force, 0.01f, 2.f);
