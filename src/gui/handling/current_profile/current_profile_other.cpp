...
namespace big
{
	void tab_current_profile::tab_other()
	{
		if (ImGui::BeginTabItem("HANDLING_TAB_OTHER"_T.data()))
		{
			...
			ImGui::SliderFloat("###multiplicador de aceleración", &g_local_player->m_vehicle->m_handling_data->m_acceleration, .01f, 10.f);

			ImGui::Text("HANDLING_DOWNFORCE_MULTIPLIER"_T.data());
			ImGui::SliderFloat("###multiplicador de fuerza descendente", &g_local_player->m_vehicle->m_handling_data->m_downforce_multiplier, .01f, 10.f);
			...
			if (ImGui::SliderFloat3("##multiplicador de inercia", fInertiaMult, -10.f, 10.f))
			{
