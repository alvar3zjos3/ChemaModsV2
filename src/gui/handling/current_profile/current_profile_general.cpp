...
namespace big
{
	void tab_current_profile::tab_general()
	{
		if (ImGui::BeginTabItem("HANDLING_TAB_GENERAL"_T.data()))
		{
			...
			ImGui::SliderFloat("##gravedad", &g_local_player->m_vehicle->m_gravity, -50.f, 50.f);

			ImGui::Text("HANDLING_MASS"_T.data());
			ImGui::SliderFloat("##masa", &g_local_player->m_vehicle->m_handling_data->m_mass, 0.f, 50000.f);
			...
			if (ImGui::SliderFloat3("##centro de masa", fCenterOfMass, -10.f, 10.f))
			{
			...
			ImGui::SliderFloat("##flotabilidad", &g_local_player->m_vehicle->m_handling_data->m_buoyancy, .01f, 99.f);
