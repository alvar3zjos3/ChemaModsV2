			...
			if (!memory::batch_runner::run(batch, mem_region))
			{
				...
				if (m_gta.m_online_version && m_gta.m_game_version)
				{
					...
					if (gta_version != m_gta_version_target)
						message = std::format("{} (Note: Found game version {} but this ChemaModsV2 version is for game version {})", message, gta_version, m_gta_version_target);
				}
				...
				{
					MessageBoxA(m_hwnd, message.c_str(), "ChemaModsV2", MB_ICONWARNING | MB_TOPMOST);
				}
