	...
	inline void dump_entry_points()
	{
		try
		{
			...
			std::unordered_map<uint64_t, uint64_t> current_native_hash_to_original;
			{
				...
				{
					LOG(FATAL) << "Need the crossmap.txt file from ChemaModsV2 repo. Put into %appdata%/ChemaModsV2 folder.";
					return;
				}
			...
			std::unordered_map<uint64_t, std::string> original_native_hash_to_name;
			{
				...
				{
					LOG(FATAL) << "Need the natives.json file from ChemaModsV2 repo. Put into %appdata%/ChemaModsV2 folder.";
					return;
				}
